...
define(["processing", "box2dweb"], function (Processing, Box2D) {;
  ...
  PBox2D.prototype.vectorWorldToPixels = function (b2vec) {
    var x = b2vec.x, y = b2vec.y;

    var pixelX = x*this.scaleFactor;
    var pixelY = y*this.scaleFactor;

    var vec = new this.processing.PVector(pixelX, pixelY);
  ...
  PBox2D.prototype.vectorPixelsToWorld = function (vec) {
    var x = vec.x, y = vec.y;

    var pixelX = x/this.scaleFactor;
    var pixelY = y/this.scaleFactor;

    var b2vec = new b2Vec2(pixelX, pixelY);
